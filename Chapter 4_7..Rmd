...
```{r}
ref <- filter(NHANES, AgeDecade == " 20-29" & Gender == "female") %>% summarize(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
...
```{r}
ref_avg <- filter(NHANES, AgeDecade == " 20-29" & Gender == "female") %>% summarize(average = mean(BPSysAve, na.rm = TRUE)) %>% pull(average)
ref_avg 
```
...
```{r}
filter(NHANES,Gender == "female")  %>% group_by(AgeDecade)%>% summarize(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
```
...
```{r}
filter(NHANES,Gender == "male")  %>% group_by(AgeDecade)%>% summarize(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
```
...
```{r}
NHANES %>% filter(AgeDecade == " 40-49" & Gender == "male")%>% group_by(Race1) %>% summarize(systolicbloodpressur=mean(BPSysAve,na.rm = T))%>% arrange(systolicbloodpressur)
```
## 4.11 Tibbles
...
dfw3 <- data.frame("n" = 1:100, "s_n" <- map_df(n, compute_s_n), "s_n_2" <- map_df(n, compute_s_n))
