...
How can we compute  Sn for various values ofn, say  n=1,…,25? Do we write 25 lines of code calling compute_s_n? No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of  
n . For-loops let us define the range that our variable takes (in our example  n=1,…,10 ), then change the value and evaluate expression as you loop.
...
In each iteration  
n=1,  n=2, etc…, we compute  Sn and store it in the  n th entry of s_n.
...
n computes the sum  Sn=1^2+2^2+3^2+…n^2
...
11. Plot  
Sn versus  n. Use points defined by  n=1,…,25 .
```{r}
for(n in 1:25){
  s_n[n] <- compute_s_n(n)
}
