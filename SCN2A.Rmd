---
title: "SCN2A mutations in neurodevelopmental disorders"
output:
  html_document:
    df_print: paged
---
...
```{r}
i <- 1:293
dpp <- function(i){gsub('[^0-9]', '', d[i, 'p.Protein'] )}
dpp1 <-sapply(i,dpp )
dpp1
```
...
```{r}
i <- 1:293
DNA <- function(i){gsub('[^0-9]', '', d[i, 'c.DNA'] )}
DNA1 <-sapply(i,DNA )
DNA1
```
...
```{r}
dd <- as.data.frame(table(d$c.DNA))
dd1 <- dd[dd$Var1 !=".", ]
dd2 <- dd1[dd1$Var1 !="???",]
nrow(dd2)
dd2
...
```{r}
library(tidyverse)
ph <- as.data.frame(table(d$Classification))
ph <- mutate (ph,prop =(ph$Freq)/sum(ph$Freq))
ph
sum(ph$prop)
```
...
sex <- d %>% 
  ...
  count()
sex <- sex[sex$PatientSex =="F"|sex$PatientSex =="M", ]
sex
```
...
```{r}
d %>% 
  group_by(Classification,p.Protein) %>%
  count()

m <- as.data.frame(table(d$p.Protein))
m
```
```{r}
d %>% 
  group_by(Classification,c.DNA) %>%
  count()
```
...
```{r}
dd12 <- as.data.frame(dpp1)
class(dpp1)
```
```{r}
library(tidyverse)
dpp2 <- as.data.frame(table(dpp1))
dpp2$dpp3 <- levels(dpp2$dpp1) %>% as.numeric()
dpp2
dpp2 %>% arrange(dpp2$dpp3)
```


```{r}
as.data.frame(table(DNA1))
```


```{r}
DNA2 <- as.data.frame(table(DNA1))
DNA2$DNA3 <- levels(DNA2$DNA1) %>% as.numeric()

DNA2 %>% arrange(DNA2$DNA3)
```
